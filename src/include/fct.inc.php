...
/**
 * Retourne un tableau contenant le sommaire
 * @param string $str
 * @return array
 */
function getSommaire($str) {
    switch ($str) {
        case 'Visiteur':
            return [
                '<ul id="menuList">',
                '<li>' . $_SESSION['statut'] . ': <br><b>' . $_SESSION['prenom'] . ' ' . $_SESSION['nom'] . '</b></li>',
                '<li class="smenu"><a href="index.php?uc=gererFrais&action=saisirFrais" title="Saisie fiche de frais">Saisie fiche de frais</a></li>',
                '<li class="smenu"><a href="index.php?uc=etatFrais&action=selectionnerMois" title="Mes fiches de frais">Mes fiches de frais</a></li>',
                '<li class="smenu"><a href="index.php?uc=connexion&action=deconnexion" title="Se déconnecter">Déconnexion</a></li>',
                '</ul>'
            ];
        case 'Comptable':
            return[
                '<ul id="menuList">',
                '<li>' . $_SESSION['statut'] . ': <br><b>' . $_SESSION['prenom'] . ' ' . $_SESSION['nom'] . '</b></li>',
                '<li class="smenu"><a href="index.php?uc=validerFrais&action=selectionnerMois" title="Valider les fiches de frais">Valider les fiches de frais</a></li>',
                '<li class="smenu"><a href="index.php?uc=suiviFrais&action=selectionnerMois" title="Suivi des fiches de frais">Suivi des fiches de frais</a></li>',
                '<li class="smenu"><a href="index.php?uc=connexion&action=deconnexion" title="Se déconnecter">Déconnexion</a></li>',
                '</ul>'
            ];
        default:
...
/**
 * retourne la liste des six derniers mois
 * 
 * @return string
 */
...
/**
 * retourne la liste des six derniers mois format numérique
 * 
 * @param type $num
 * @return array
 */
function getMoisAvecNumMois($num) {
    $tabMois = array(
        "01" => array("mois_num" => "01", "mois_fr" => "Janvier"),
        "02" => array("mois_num" => "02", "mois_fr" => "Février"),
        "03" => array("mois_num" => "03", "mois_fr" => "Mars"),
    ...
    return $tabMois[$num]['mois_fr'];
}

/**
 * 
 * retourne option select visiteur formaté
 * 
 * @param array $visiteurs
 * @return string
 */
function setVisiteurs(array $visiteurs) {
    foreach ($visiteurs as $unVisi) {
        $tabVisi[] = '<option value="' . $unVisi['id'] . '">' . $unVisi['nom'] . " " . $unVisi['prenom'] . '</option>';
    }
    return $tabVisi;
}
/**
